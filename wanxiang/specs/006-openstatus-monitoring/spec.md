...
3. **Given** public traffic follows the cluster network policy, **When** the private location checks a public endpoint, **Then** the implementation follows that policy and does not introduce a direct exception.
...
- Existing Flux, Gateway API, Cloudflare Tunnel, SOPS, Longhorn, and cluster network patterns remain available on talos-ii.
