...
The original spec walkthrough and fold-back notes are archived in the
private infrastructure repository.

The upstream network contract this runner consumes is maintained in the private
infrastructure repository. The image pull chain is documented in
[`zot-mirror.md`](zot-mirror.md) (LAN host,
...
First verify the cluster upstream network policy using its private runbook.

```bash
# 1. Cluster-local Service-name targets resolve.
...
# 2. LAN zot reachable from a talos-ii node.
...
local plain HTTP; the cluster network policy keeps cluster-local
traffic direct). `CONFIG_NAME` is
...
# Verify upstream network compatibility with the checks in the private
...
If public fetches time out while cluster-local services remain healthy,
follow the private network runbook. Confirm `hostNetwork: true` still appears
on the runner Pod before investigating the separately managed network path.
...
Cause: upstream WAF anti-bot uses TLS fingerprint detection. Both the
configured upstream path and a direct connection fail the fingerprint
check; this is unrelated to the upstream network implementation.

Mitigation today: those derivations are **disabled** in the
`nix-fleet` matrix. Re-enable when an upstream-compatible client is
available (tracked separately).
