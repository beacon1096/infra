...
- Cluster upstream connectivity is healthy; verify it using the private
  network runbook.
...
Likely the cluster upstream network path is unhealthy and `extensions.sync`
...
Inspect `kubectl -n registry logs zot-0 --tail=200` for sync failures,
then follow the private network runbook.

The network runbook is maintained in the private infrastructure repository.
While upstream connectivity is being repaired, the LAN host fallback covers
talos-ii pulls; the operator does NOT need to roll back the
machine-config flip.

### C. LAN host zot ALSO unreachable, in-cluster has no cache

Worst-case-but-not-catastrophic. New image pulls use the private
upstream path directly and may be slower, but they succeed. The
operator should:
...
3. The cluster upstream network foundation starts according to its
   private recovery runbook.
4. zot starts and fetches its pinned image through that path. It may
   report `ImagePullBackOff` briefly while dependencies stabilize.
5. Once zot is Ready, all subsequent pulls flip to in-cluster.

If zot stays in `ImagePullBackOff` indefinitely after a cold start,
verify upstream network reachability first.
