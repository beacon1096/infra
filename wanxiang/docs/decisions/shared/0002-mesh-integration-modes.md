...
5. **Operational complexity at steady state** — existing private
   networking policy already carries custom integration points; adding
   more controllers should be justified.
...
2. **Repeated egress-reachability incidents** affecting network-dependent
   build tools. The detailed remediation is maintained with the private
   network policy, and assumes
...
ADRs (talos-i positioning, private network policy, VLAN-LB rewrite) build
on top of this one.
