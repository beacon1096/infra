...
backup/DR strategy, private network policy) can build on a stable
premise.
...
- `development/forgejo-runner` — already under discussion as part
  of the CI-runner consolidation thread
...
5. **Timing** — talos-i adoption + offsite move happens **after**
   ...
   - private network policy implemented and stable on talos-ii
...
(private network policy, VLAN-LB rewrite, Backup/DR) can be
