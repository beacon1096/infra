...
in the runbook prose: it predated the current cluster network policy,
the Cilium L2 announcement pool, and the Longhorn storage tier on
...
baseline. attic (0011), n8n / coder (0010), forgejo, and vaultwarden
use `bjw-s app-template`. zot does not.
...
- The 91 MiB download has to traverse the upstream egress path, which
...
### 7. Upstream network policy

zot's `extensions.sync` calls follow the cluster upstream network policy.
Detailed network settings and incident history are maintained in the private
infrastructure repository. The public contract is limited to one invariant:
cluster-local destinations remain direct.
...
- **Self-referential `"172.16.80.240:5000"`**: kept. Required
  ...
  `172.16.80.240:5000/foo:bar` (including bootstrap images)
  continues to work plain-HTTP. Removed only at
...
- Private zot implementation archive — spec, plan, research, data model, quickstart, and contracts
