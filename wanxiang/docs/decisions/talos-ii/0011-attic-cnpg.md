...
next rebuild, or re-fetched from `cache.nixos.org` through the cluster
upstream network path on first miss. The cost of importing the
...
isn't the bottleneck; upstream bandwidth is the bottleneck for
first-fill, and that
...
`PrerequisitesNotMet` forever. The same gotcha is retained in the
private implementation archive.
...
- Private Attic implementation archive — spec and fold-back lessons
