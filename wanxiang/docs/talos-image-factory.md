...
policy-routing rules that conflicted with the cluster egress path. The
superseded ADR and incident runbook are archived in the private
infrastructure repository.

Current Tailscale usage is in-cluster only: the Tailscale operator
handles per-Service tailnet exposure. Additional egress integration is
...
installed policy-routing rules that conflicted with the cluster network path.
ADR `talos-ii/0014` was superseded and
