...
### 本机异步 Bash 试用（2026-09-15）

试用中，模型为两台机器的 NVIDIA 模块构建显式传入 `timeout: 300`，
原生 Bash 在下载与远程产物回传期间到期终止命令。该值来自工具调用，
不是 Pi 的默认超时，也不是推理接口的请求超时。

新增 infra-private 仓内 Nix 管理的 `modules/home/pi-exec/extension.ts`，覆盖 `bash`：

- 十秒内完成直接返回；超过十秒自动返回任务 ID，命令继续执行。
- `background: true` 立即返回；完成后以 followUp 消息触发模型续跑，无需轮询。
- `timeout` 仍是秒数形式的硬截止，不是 yield 间隔。默认不设硬截止，长构建应省略，除非确有取消期限。
- `exec_status` 查看任务状态与日志路径，`exec_cancel` 终止任务及进程树。
- 仅 `print/json` 模式通过 awaited `agent_end` 等待任务完成并排队通知，Pi 0.85.1 随后消费通知再退出。
  此实现自行管理 exec 生命周期，不依赖整套 pi-subagents，也不替代上层工单调度。
- 原生 Pi Bash 操作负责 Shell、环境与进程树清理；真正关闭会话或成功切换会话时清理后台任务。
  已后台化任务不因当前模型回合结束而取消；取消一次会话导航不会提前杀任务。

单任务磁盘日志最多 64 MiB（保留开头），模型消息保留约 50 KiB 尾部，
超限有明确标记；内存保留最近一百项已结束任务。日志位于权限受限的
`/tmp/pi-exec-*`，扩展不自动删除历史日志，也不提供进程重启后的任务恢复。
未设 timeout 且永不结束的命令会让 headless 持续等待，需要取消或显式设定期限。
完成通知没有持久化 ACK/重试协议；调用投递失败会记录错误，不能仅以 Pi exit 0 判定全部任务已交付。

使用真实 Pi CLI 与本地可控模型服务通过八项验收：成功、退出失败、硬超时、取消、
十一秒命令自动 yield、快命令不重复通知、两个后台任务结果均消费，以及取消后孙进程不再写入哨兵。
测试脚本：infra-private 仓内 `modules/home/pi-exec/tests/smoke.py`，依赖本机 `pi` 与 Python 标准库。
这些测试验证实际工具与 CLI 生命周期，未把模型服务的模拟响应当作真实模型的使用习惯验收。

仅在 msi-claw 激活由 Nix 构建并保留 GC root 的扩展文件，未进行全系统 switch，
避免夹带其他 Pi 正在修改的 NVIDIA 配置。已有 Pi 会话需在当前命令结束后 `/reload`，
新会话自动加载；Coder、Multica 与其他机器需要后续部署才会启用此扩展。

### Paseo RPC 兼容修正（2026-09-15）

本机 Paseo 0.3.1 试用发现新输入报 `Agent is already processing`。
该适配层收到 `agent_end` 就标记回合完成，但 Pi 0.85.1 在扩展 handler 与
post-run 消息处理完成后才发出 `agent_settled`。原试用扩展还将 RPC 与 print
一同 drain，延长了客户端认为空闲、Pi 实际仍忙碌的窗口。

扩展已改为只在 print/json drain；RPC 是常驻宿主，后台完成仍可主动续跑，
无需堵住 `agent_end`。真实 RPC 验收确认：等待 `agent_settled` 后，新普通 prompt
在后台命令完成前响应，随后命令完成通知仍触发模型；print 成功及自动 yield 回归通过。
Paseo 0.3.1 的本地补丁改为缓存 `agent_end.messages`，到 `agent_settled` 才完成回合；
prompt 使用 `streamingBehavior: "followUp"` 防止与后台回调启动竞争，
无活跃回合时的自动续跑获得独立回合 ID。补丁针对当前 Pi 0.85.1，
没有为更旧的 Pi 引入轮询兼容。相关 agent/runtime 测试 76 项全部通过。
补丁由私有仓 `lib/paseo` 统一供 daemon 与 desktop 的 Nix 包使用；
msi-claw 的系统基底来自公开 infra，因此私有覆盖需明确导入才会作用到实际服务。
扩展已单独激活；修正版 Paseo daemon 已完成 Nix 构建，安装产物已核对包含
`agent_settled` 与 `streamingBehavior` 修正。daemon 尚未替换运行实例，
因为当前操作会话本身依赖它；需在对话结束后完成系统配置激活与 daemon 重启。

### Luna 试用审计后的修复（2026-09-15）

本次审计的 Paseo 会话使用 Luna/high。
观察到与已读源码冲突的 schema 误报、重复等待轮询、大量上下文输入，
以及删除 Paseo 回归测试的问题。该次表现不足以推导模型在两种 harness 中的普遍差异。

复核后保留有效的 Paseo custom message 完成条件修正，恢复独立测试补丁，
从修改后的上游源码重新生成带上下文的 patch。新增用例确认 custom message
不会提前结束活跃模型回合；相关测试共 77 项通过。

另发现 exec 正常输出向 Pi 原生结果收集器的转发被删，命令成功也可能没有输出。
修复该回归，并将测试改为模型服务实际收到唯一输出标记才确认成功，
不再仅验证任务状态和静态确认文本。

本轮将默认 Bash 结果尾部收紧为 4 KiB、最多 80 行，状态查询默认只返回元数据，
短时间内完成的任务合并通知。完整日志仍有 64 MiB 上限，详细信息按需读取。
Pi 专用上下文补充：审查前核对准确源码、按指定会话交接、保留等价测试、
从源码生成补丁、限制搜索范围、构建前检查平台能力，以及等待完成通知和主动报告进度。
这些规则用于改善执行约束，不代表已经证明 Luna 的任务成功率提升。
最终 exec 12 项真实 CLI 场景全部通过，包括真实输出进入模型请求、状态查询去除日志、
通知合并、截断与磁盘日志上限；针对已激活 Nix store 文件的成功路径再次通过。
修复后的 Paseo 包也完成 Nix 构建；本轮仅激活本机 Pi 扩展与规则，没有重启 Paseo daemon。
