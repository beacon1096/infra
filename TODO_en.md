...
- [ ] Mark `Check Docs i18n` as a required status in the Forgejo branch
  protection for `main`.
- [x] Migrate the repo entrypoint: `README.md` becomes the Chinese version
  and `README_en.md` is added, both with a language-toggle header link.
- [x] Migrate `AGENTS.md` and `TODO.md` themselves: Chinese source of truth
  plus `AGENTS_en.md` / `TODO_en.md` translations, each with a
  language-toggle header link.
- [ ] Fix the stale `scripts/deploy-nix-darwin.sh` reference in
  `docs/deploy-nix-darwin.md` (the script actually lives in `utils/`) when
  migrating that document.
